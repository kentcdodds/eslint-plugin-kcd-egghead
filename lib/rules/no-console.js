      ...
      'Program:exit'() {
        consoleUsage.forEach(identifier => {
          if (isDisallowedFunctionCall(identifier)) {
            ...
            const variableDeclaratorParent = findParent(identifier, parent => parent.type === 'VariableDeclarator')
            if (variableDeclaratorParent) {
              const references = context.getDeclaredVariables(variableDeclaratorParent)[0].references.slice(1)
    ...
    function isDisallowedFunctionCall(identifier) {
      return looksLike(identifier, {
        parent: {
          ...
          property: {
            name: val => !allowedMethods.includes(val) && disallowedMethods.includes(val),
          },
        },
      })
    }
